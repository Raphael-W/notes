...
def handleTextUpdate(newText):
    ...
    currentText = newText
    emit('update_text', newText, broadcast=True, include_self = False)  # Send updated text to all clients
    updateTextFile(newText)

@socketio.on('connect')
def send_current_text():
    emit('update_text', currentText)  # Send the current text to a newly connected client
